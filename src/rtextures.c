...
Image ImageFromImage(Image image, rayRect rec)
{
    Image result = { 0 };
...
void ImageCrop(Image *image, rayRect crop)
{
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        else
        {
            ...
            {
                rayRect rec = { (float)(textOffsetX + font.glyphs[index].offsetX), (float)(textOffsetY + font.glyphs[index].offsetY), (float)font.recs[index].width, (float)font.recs[index].height };
                ImageDraw(&imText, font.glyphs[index].image, (rayRect){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
...
void ImageAlphaCrop(Image *image, float threshold)
{
    ...
    rayRect crop = GetImageAlphaBorder(*image, threshold);
...
void ImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill)
{
    ...
    {
        rayRect srcRec = { 0, 0, (float)image->width, (float)image->height };
...
// NOTE: Threshold is defined as a percentatge: 0.0f -> 1.0f
rayRect GetImageAlphaBorder(Image image, float threshold)
{
    rayRect crop = { 0 };
    ...
    if (pixels != NULL)
    {
        ...
        {
            crop = (rayRect){ (float)xMin, (float)yMin, (float)((xMax + 1) - xMin), (float)((yMax + 1) - yMin) };
        }
...
{
    ImageDrawRectangleRec(dst, (rayRect){ (float)posX, (float)posY, (float)width, (float)height }, color);
}
...
// Draw rectangle within an image
void ImageDrawRectangleRec(Image *dst, rayRect rec, Color color)
{
    // Security check to avoid program crash
...
void ImageDrawRectangleLines(Image *dst, rayRect rec, int thick, Color color)
{
...
void ImageDraw(Image *dst, Image src, rayRect srcRec, rayRect dstRec, Color tint)
{
    // Security check to avoid program crash
    ...
    else
    {
        ...
        if (((int)srcRec.width != (int)dstRec.width) || ((int)srcRec.height != (int)dstRec.height))
        {
            ...
            srcRec = (rayRect){ 0, 0, (float)srcMod.width, (float)srcMod.height };
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    ...
    rayRect srcRec = { 0.0f, 0.0f, (float)imText.width, (float)imText.height };
    rayRect dstRec = { position.x, position.y, (float)imText.width, (float)imText.height };
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        rayRect faceRecs[6] = { 0 };      // Face source rectangles
        for (int i = 0; i < 6; i++) faceRecs[i] = (rayRect){ 0, 0, (float)size, (float)size };
        ...
        for (int i = 0; i < 6; i++) ImageDraw(&faces, image, faceRecs[i], (rayRect){ 0, (float)size*i, (float)size, (float)size }, WHITE);
...
// NOTE: pixels data must match texture.format
void UpdateTextureRec(Texture2D texture, rayRect rec, const void *pixels)
{
...
{
    rayRect source = { 0.0f, 0.0f, (float)texture.width, (float)texture.height };
    rayRect dest = { position.x, position.y, (float)texture.width*scale, (float)texture.height*scale };
...
void DrawTextureRec(Texture2D texture, rayRect source, Vector2 position, Color tint)
{
    rayRect dest = { position.x, position.y, fabsf(source.width), fabsf(source.height) };
    Vector2 origin = { 0.0f, 0.0f };
...
void DrawTextureQuad(Texture2D texture, Vector2 tiling, Vector2 offset, rayRect quad, Color tint)
{
    rayRect source = { offset.x*texture.width, offset.y*texture.height, tiling.x*texture.width, tiling.y*texture.height };
    Vector2 origin = { 0.0f, 0.0f };
...
void DrawTextureTiled(Texture2D texture, rayRect source, rayRect dest, Vector2 origin, float rotation, float scale, Color tint)
{
    ...
    if ((dest.width < tileWidth) && (dest.height < tileHeight))
    {
        ...
        DrawTexturePro(texture, (rayRect){source.x, source.y, ((float)dest.width/tileWidth)*source.width, ((float)dest.height/tileHeight)*source.height},
                    (rayRect){dest.x, dest.y, dest.width, dest.height}, origin, rotation, tint);
    }
    else if (dest.width <= tileWidth)
    {
        ...
        {
            DrawTexturePro(texture, (rayRect){source.x, source.y, ((float)dest.width/tileWidth)*source.width, source.height}, (rayRect){dest.x, dest.y + dy, dest.width, (float)tileHeight}, origin, rotation, tint);
        }

        // Fit last tile
        if (dy < dest.height)
        {
            DrawTexturePro(texture, (rayRect){source.x, source.y, ((float)dest.width/tileWidth)*source.width, ((float)(dest.height - dy)/tileHeight)*source.height},
                        (rayRect){dest.x, dest.y + dy, dest.width, dest.height - dy}, origin, rotation, tint);
        }
    }
    else if (dest.height <= tileHeight)
    {
        ...
        {
            DrawTexturePro(texture, (rayRect){source.x, source.y, source.width, ((float)dest.height/tileHeight)*source.height}, (rayRect){dest.x + dx, dest.y, (float)tileWidth, dest.height}, origin, rotation, tint);
        }

        // Fit last tile
        if (dx < dest.width)
        {
            DrawTexturePro(texture, (rayRect){source.x, source.y, ((float)(dest.width - dx)/tileWidth)*source.width, ((float)dest.height/tileHeight)*source.height},
                        (rayRect){dest.x + dx, dest.y, dest.width - dx, dest.height}, origin, rotation, tint);
        }
    }
    else
    {
        ...
        for (;dx+tileWidth < dest.width; dx += tileWidth)
        {
            ...
            for (;dy+tileHeight < dest.height; dy += tileHeight)
            {
                DrawTexturePro(texture, source, (rayRect){dest.x + dx, dest.y + dy, (float)tileWidth, (float)tileHeight}, origin, rotation, tint);
            }

            if (dy < dest.height)
            {
                DrawTexturePro(texture, (rayRect){source.x, source.y, source.width, ((float)(dest.height - dy)/tileHeight)*source.height},
                    (rayRect){dest.x + dx, dest.y + dy, (float)tileWidth, dest.height - dy}, origin, rotation, tint);
            }
        }
        ...
        if (dx < dest.width)
        {
            ...
            for (;dy+tileHeight < dest.height; dy += tileHeight)
            {
                DrawTexturePro(texture, (rayRect){source.x, source.y, ((float)(dest.width - dx)/tileWidth)*source.width, source.height},
                        (rayRect){dest.x + dx, dest.y + dy, dest.width - dx, (float)tileHeight}, origin, rotation, tint);
            }

            // Draw final tile in the bottom right corner
            if (dy < dest.height)
            {
                DrawTexturePro(texture, (rayRect){source.x, source.y, ((float)(dest.width - dx)/tileWidth)*source.width, ((float)(dest.height - dy)/tileHeight)*source.height},
                    (rayRect){dest.x + dx, dest.y + dy, dest.width - dx, dest.height - dy}, origin, rotation, tint);
            }
        }
    }
}
...
void DrawTexturePro(Texture2D texture, rayRect source, rayRect dest, Vector2 origin, float rotation, Color tint)
{
